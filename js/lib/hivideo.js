...
(function(global, factory){
    ...
    hivideo.prototype = {
        ...
        config: function(){
            ...
            //初始化事件注册数组，eventArr包含了所有事件类型以及事件类型注册的事件集合
            self.events = self.events || {};
            ["fullscreenchange"].forEach(function(name){
                if(!self.events[name]){
                    self.events[name] = [];
                }
            });
    ...
    hivideo.prototype.bindFullEvent = function(){
        ...
        ["fullscreenchange", "webkitfullscreenchange", "mozfullscreenchange", "MSFullscreenChange"].forEach(function(eventType){
            ...
            document.addEventListener(eventType, function(event){
                ...
                //回调用户注册的fullscreenchange事件
                self.events["fullscreenchange"].forEach(function(callback){
                    callback(event);
                });
            })
        });
...
//媒体数据加载完成
     ...
     * 注册事件
     */
    hivideo.prototype.on = function(event, callback){
        var self = this;
